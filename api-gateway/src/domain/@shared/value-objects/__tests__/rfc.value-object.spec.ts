...
describe('RFC Value Object', () => {
  beforeEach(() => {
    // Always register a validator before each test
  ...
  it('should throw InvalidRFCError with reason INVALID_FORMAT if validator fails', () => {
    // Register a failing validator for this specific test
    ...
    expect(() => {
      ...
      new InvalidRFCError(
        invalidRFC.toUpperCase(),
        RFCValidationReason.InvalidFormat,
      ),
    );
  });
