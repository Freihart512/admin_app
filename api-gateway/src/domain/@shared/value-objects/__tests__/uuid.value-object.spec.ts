...
import type { UUIDValidator } from '../../ports/uuid.validator.port';

const validUUID = '123e4567-e89b-12d3-a456-426614174000';
const invalidUUID = 'not-a-uuid';

// Utilidad para mockear el validador
function mockValidator(valid: boolean): UUIDValidator {
  return {
...
describe('UUID Value Object', () => {
  it('should create UUID successfully with a valid UUID', () => {
    ...
    const uuid = UUID.create(validUUID.toUpperCase(), validator);
  ...
  it('should throw InvalidUUIDError if the UUID is invalid', () => {
    ...
    expect(() => {
      UUID.create(invalidUUID, validator);
    }).toThrowError(new InvalidUUIDError(invalidUUID));
  });

  it('should consider two UUIDs with the same value as equal', () => {
    const validator = mockValidator(true);
    const uuid1 = UUID.create(validUUID, validator);
    const uuid2 = UUID.create(validUUID.toUpperCase(), validator);
  ...
  it('should consider two different UUIDs as not equal', () => {
    const validator = mockValidator(true);
    const uuid1 = UUID.create(validUUID, validator);
    const uuid2 = UUID.create('123e4567-e89b-12d3-a456-426614174999', validator);
