...
export class PostgresUserRepository implements IUserRepository {
  ...
  async findByEmail(email: string): Promise<User | null> {
    console.log(`PostgresUserRepository: Finding user by email: ${email}`);
    try {
  ...
  async findById(id: string): Promise<User | null> {
      console.log(`PostgresUserRepository: Finding user by id: ${id}`);
       try {
        const result: QueryResult = await this.pool.query(
  ...
  async save(user: User): Promise<void> {
       console.log(`PostgresUserRepository: Saving user: ${user.email}`);
       try {
        const query = `
  ...
  async update(user: User): Promise<void> {
       console.log(`PostgresUserRepository: Updating user: ${user.id}`);
       try {
         const query = `
           UPDATE users
         ...
         if (error instanceof UserNotFoundError) {
          console.log('Caught UserNotFoundError in catch, re-throwing');
          throw error;
      }
          if (error.code === '23505') {
            ...
            if (error.constraint && error.constraint.includes('users_rfc_') && user.rfc) {
             ...
             if (error.constraint && error.constraint.includes('users_phone_number_') && user.phoneNumber) {
                throw new Error(`Phone number already exists.`);
            }
         }
  ...
  async updatePassword(userId: string, hashedPassword: string): Promise<void> {
       console.log(`PostgresUserRepository: Updating password for user: ${userId}`);
       try {
         const query = `
           UPDATE users
           SET password_hash = $1
  ...
  async savePasswordResetToken(userId: string, token: string, expiresAt: Date): Promise<void> {
      console.log(`PostgresUserRepository: Saving password reset token for user ${userId}`);
      try {
        const query = `
        ...
        console.error(`Error saving password reset token for user ${userId}:`, error);
         ...
         throw new DatabaseError(`Failed to save password reset token for user ${userId}: ${error.message}`, {
            type: 'INSERT_ERROR',
            dbErrorCode: error.code,
  ...
  async findPasswordResetToken(token: string): Promise<{ userId: string, expiresAt: Date } | null> {
       console.log(`PostgresUserRepository: Finding password reset token: ${token}`);
       try {
         const result: QueryResult = await this.pool.query(
  ...
  async invalidatePasswordResetToken(token: string): Promise<void> {
       console.log(`PostgresUserRepository: Invalidating password reset token: ${token}`);
       try {
         const query = `
   ...
   async updateStatus(userId: string, isActive: boolean): Promise<void> {
       console.log(`PostgresUserRepository: Updating status for user ${userId} to ${isActive}`);
       try {
         const query = `
           UPDATE users
           SET is_active = $1
   ...
   async updateRoles(userId: string, roles: string[]): Promise<void> {
       console.log(`PostgresUserRepository: Updating roles for user ${userId}: ${roles.join(', ')}`);
       try {
         const query = `
           UPDATE users
           SET roles = $1
