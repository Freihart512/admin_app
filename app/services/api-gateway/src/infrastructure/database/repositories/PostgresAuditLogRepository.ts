...
export class PostgresAuditLogRepository implements IAuditLogRepository {
  ...
  async saveEvent(event: IDomainEvent): Promise<void> {
    console.log(`PostgresAuditLogRepository: Saving event to audit logs: ${event.type}`);
    try {
      const query = `
      ...
      console.log(`Audit log saved for event: ${event.type}`);
      ...
      throw new DatabaseError(`Failed to save audit log for event ${event.type}: ${error.message}`, {
          type: 'INSERT_ERROR',
