...
const authenticationResolvers = {
  Mutation: {
    login: async (_: any, { input }: { input: LoginInput }): Promise<LoginPayload> => {
      console.log('GraphQL Resolver: login mutation called with email:', input.email);
      try {
    ...
    requestPasswordReset: async (_: any, { input }: { input: RequestPasswordResetInput }): Promise<MutationResponse> => {
      console.log('GraphQL Resolver: requestPasswordReset mutation called for email:', input.email);
       try {
         ...
         // Devolver datos de prueba o el resultado real del caso de uso
          console.log('RequestPasswordResetUseCase placeholder called.');
          return {
            success: true,
    ...
    resetPassword: async (_: any, { input }: { input: ResetPasswordInput }): Promise<MutationResponse> => {
       console.log('GraphQL Resolver: resetPassword mutation called with token:', input.token);
       try {
         // Llamar al caso de uso ResetPasswordUseCase
         // const result = await resetPasswordUseCase.execute(input);
         // Devolver datos de prueba o el resultado real del caso de uso
          console.log('ResetPasswordUseCase placeholder called.');
    ...
    changePassword: async (_: any, { input }: { input: ChangePasswordInput }): Promise<MutationResponse> => {
       console.log('GraphQL Resolver: changePassword mutation called.');
        try {
          // Llamar al caso de uso ChangePasswordUseCase
          // const result = await changePasswordUseCase.execute(input);
          // Devolver datos de prueba o el resultado real del caso de uso
           console.log('ChangePasswordUseCase placeholder called.');
           if (input.newPassword !== input.confirmNewPassword) {
              return { success: false, message: "New passwords don't match." };
           }
           ...
           console.log('User authentication and authorization logic would be needed here.');
           return {
             success: true,
