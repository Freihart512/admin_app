...
describe('PostgresUserRepository (Infrastructure Integration Test)', () => {
  ...
  it('should throw DatabaseError on database query error in findByEmail', async () => {
    ...
    await expect(postgresUserRepository.findByEmail(email)).rejects.toThrow(
    ...
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('SELECT'), [email]);
  });
  ...
  it('should return null when finding by id and user does not exist', async () => {
    ...
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('SELECT'), [nonExistentId]);
    expect(result).toBeNull();
  });
  ...
  it('should throw DatabaseError on database query error in findById', async () => {
    ...
    await expect(postgresUserRepository.findById(userId)).rejects.toThrow(
      expect.objectContaining({
    ...
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('SELECT'), [userId]);
  });
  ...
  it('should save a user successfully', async () => {
    ...
    const newUser = new User(
      '8c7e92ee-1124-48c6-bd46-8388e8aab6b1',
      'newuser@example.com',
      'New',
      'User',
      '0987654321',
  ...
  it('should throw EmailAlreadyExistsError on email unique constraint violation', async () => {
    ...
    await expect(postgresUserRepository.save(newUser)).rejects.toThrow(
      expect.objectContaining({
  ...
  it('should throw RfcAlreadyExistsError on rfc unique constraint violation', async () => {
    const pgError = new Error(
      'duplicate key value violates unique constraint "users_rfc_key"'
    ) as any; // Ajusta el nombre si es diferente
    pgError.code = '23505';
  ...
  it('should update a user successfully', async () => {
    ...
    const updatedUser = new User(
      ...
      'UPDATEDRFC',
      ['Propietario'],
      testUserEntity.passwordHash,
      false // Cambiamos algunos campos
    );
    ...
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE'), [
      ...
      updatedUser.id, // ID al final para la cláusula WHERE
    ]);
  });
  ...
  it('should throw UserNotFoundError when updating a user that does not exist', async () => {
    ...
    await expect(postgresUserRepository.update(nonExistentUser)).rejects.toThrow(
      expect.objectContaining({
  ...
  it('should throw EmailAlreadyExistsError on email unique constraint violation during update', async () => {
    ...
    const userToUpdate = new User(
      ...
      'hash',
      true
    ); // Email que ya existe para otro usuario

    await expect(postgresUserRepository.update(userToUpdate)).rejects.toThrow(
  ...
  it('should throw RfcAlreadyExistsError on rfc unique constraint violation during update', async () => {
    ...
    const userToUpdate = new User(
      ...
      'hash',
      true
    ); // RFC que ya existe para otro usuario

    await expect(postgresUserRepository.update(userToUpdate)).rejects.toThrow(
      RfcAlreadyExistsError
    );
    await expect(postgresUserRepository.update(userToUpdate)).rejects.toThrow(
      expect.objectContaining({
  ...
  it('should throw UserNotFoundError when updating password for a user that does not exist', async () => {
    ...
    await expect(
      postgresUserRepository.updatePassword(nonExistentUserId, newHashedPassword)
  ...
  it('should throw DatabaseError on generic database error in updatePassword', async () => {
    ...
    await expect(postgresUserRepository.updatePassword(userId, newHashedPassword)).rejects.toThrow(
      DatabaseError
    );
    await expect(postgresUserRepository.updatePassword(userId, newHashedPassword)).rejects.toThrow(
      expect.objectContaining({
        message: expect.stringContaining(
          `Failed to update password for user ${userId}: ${dbError.message}`
        ),
        type: 'UPDATE_ERROR',
        originalError: dbError,
      })
  ...
  it('should throw DatabaseError on generic database error in savePasswordResetToken', async () => {
    ...
    await expect(
      postgresUserRepository.savePasswordResetToken(userId, token, expiresAt)
    ...
    await expect(
      ...
      expect.objectContaining({
        message: expect.stringContaining(
          `Failed to save password reset token for user ${userId}: ${dbError.message}`
        ),
        type: 'INSERT_ERROR',
        originalError: dbError,
      })
  ...
  it('should throw DatabaseError on generic database error in findPasswordResetToken', async () => {
    ...
    await expect(postgresUserRepository.findPasswordResetToken(token)).rejects.toThrow(
      DatabaseError
    );
    await expect(postgresUserRepository.findPasswordResetToken(token)).rejects.toThrow(
      expect.objectContaining({
    ...
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('SELECT'), [token]);
  });
  ...
  it('should invalidate password reset token successfully', async () => {
    ...
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE'), [token]);
  });
  ...
  it('should throw DatabaseError on generic database error in invalidatePasswordResetToken', async () => {
    ...
    await expect(postgresUserRepository.invalidatePasswordResetToken(token)).rejects.toThrow(
      DatabaseError
    );
    await expect(postgresUserRepository.invalidatePasswordResetToken(token)).rejects.toThrow(
      expect.objectContaining({
        message: expect.stringContaining(
          `Failed to invalidate password reset token: ${dbError.message}`
        ),
  ...
  it('should update user status successfully', async () => {
    ...
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE'), [isActive, userId]);
  });
  ...
  it('should throw UserNotFoundError when updating status for a user that does not exist', async () => {
    ...
    await expect(postgresUserRepository.updateStatus(nonExistentUserId, isActive)).rejects.toThrow(
      UserNotFoundError
    );
    await expect(postgresUserRepository.updateStatus(nonExistentUserId, isActive)).rejects.toThrow(
      expect.objectContaining({
  ...
  it('should throw DatabaseError on generic database error in updateStatus', async () => {
    ...
    await expect(postgresUserRepository.updateStatus(userId, isActive)).rejects.toThrow(
      DatabaseError
    );
    await expect(postgresUserRepository.updateStatus(userId, isActive)).rejects.toThrow(
      expect.objectContaining({
        message: expect.stringContaining(
          `Failed to update status for user ${userId}: ${dbError.message}`
        ),
        type: 'UPDATE_ERROR',
        originalError: dbError,
      })
  ...
  it('should update user roles successfully', async () => {
    ...
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE'), [newRoles, userId]);
  });
  ...
  it('should throw UserNotFoundError when updating roles for a user that does not exist', async () => {
    ...
    await expect(postgresUserRepository.updateRoles(nonExistentUserId, newRoles)).rejects.toThrow(
      UserNotFoundError
    );
    await expect(postgresUserRepository.updateRoles(nonExistentUserId, newRoles)).rejects.toThrow(
      expect.objectContaining({
  ...
  it('should throw DatabaseError on generic database error in updateRoles', async () => {
    ...
    await expect(postgresUserRepository.updateRoles(userId, newRoles)).rejects.toThrow(
      DatabaseError
    );
    await expect(postgresUserRepository.updateRoles(userId, newRoles)).rejects.toThrow(
      expect.objectContaining({
        message: expect.stringContaining(
          `Failed to update roles for user ${userId}: ${dbError.message}`
        ),
        type: 'UPDATE_ERROR',
        originalError: dbError,
      })
