...
const columnDefinitions: ColumnDefinitions = {
  ...
  event_type: { // El tipo de evento (ej. 'UserLoggedInEvent')
    type: 'varchar(255)', // O un tipo ENUM si tienes un conjunto fijo de tipos de eventos
    notNull: true,
  },
  user_id: { // Campo explícito para el ID del usuario asociado al evento (puede ser NULL)
    type: 'uuid',
  ...
  payload: { // Los datos del evento en formato JSON
